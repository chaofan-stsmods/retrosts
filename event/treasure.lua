...
function TreasureEvent:drawBackground()
    ...
    if self.type == 'small' then
        sprmap(12,46,4,3,128,64,0)
    elseif self.type == 'medium' then
        sprmap(16,45,4,4,128,56,0)
    elseif self.type == 'large' then
        sprmap(20,46,5,3,128,64,8)
    end
end
...
function TreasureEvent:showRewards()
    ...
    if not sapphireKeyObtained then
	    table.insert(rewards,{title='Sapphire key',icon=463,type='key',value='sapphireKeyObtained',link=rewards[#rewards],showLink=true})
