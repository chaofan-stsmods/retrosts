...
5: low 8 bits max energy, 8~15 bits shop remove count, 16 rubyKey, 17 emeraldKey, 18, sapphireKey, 19 room completed
6: one time events
...
10: reward generator
11: event generator
12: currentX, currentY each 8 bits num potions 16 bits
13 ~ 14: player path, each item 3 bits
15: num cards, num relics, each 16 bits
16 ~ N: cards
N+1 ~ N+M: relics
N+M+1 ~ N+M+L: potions
]]--
...
local eventRoomTypes = {'event','monster','shop','treasure'}
function saveGame(completeRoom)
	local index = 0
	...
	pmem(index+5,maxEnergy | (shopRemoveCount << 8) | (rubyKeyObtained and (1<<16) or 0) |
		...
		(completeRoom and (1<<19) or 0) | ((table.indexOf(eventRoomTypes,room.eventType) or 0) << 20))
	...
	pmem(index+12,currentRoomX | (currentRoomY << 8) | (#potions << 16))
	local pathVal = 0
...
function loadGame()
	...
	local roomCompleted = val32 & (1<<19) ~= 0
	local eventRoomType = eventRoomTypes[(val32 >> 20) & 0x7]
	...
	trace(table.concat(playerPath,','))

	-- other game

	val32 = pmem(index+15)
	local deckSize = val32 & 0xffff
	local relicSize = val32 >> 16
	...
	if floor ~= 0 then
		...
		prepareRoom(roomCompleted,eventRoomType)
		window:onWindowOpen()

		if roomCompleted then
			if roomActionType == 'event' then
